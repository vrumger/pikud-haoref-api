...
function getHFCAlertsJSON(options, callback) {
    ...
    options.url = options.alertsHistoryJson
        ? config.hfc.alertsHistory.api
        : config.hfc.alerts.api;
    ...
    axios(options)
        .then(function (res) {
            ...
            if (res.status != 200) {
                return callback(
                    new Error(
                        'Failed to retrieve alerts from HFC API: ' +
                            res.status +
                            ' ' +
                            res.statusText,
                    ),
            ...
            if (buffer.toString('utf8').indexOf('/errorpage_adom/') != -1) {
                return callback(
                    new Error('The HFC API returned a temporary error page.'),
                );
            }
        ...
        .catch(function (err) {
            // Request failed?
            return callback(
                new Error('Failed to retrieve alerts from HFC API: ' + err),
            );
        });
}
...
function getAlertTypeByHistoricalCategory(category) {
    ...
    switch (category) {
        ...
        default:
            // Log this
            console.log(
                '[Pikud Haoref API] Unknown historical category: ' + category,
            );
            return 'unknown';
    }
}
