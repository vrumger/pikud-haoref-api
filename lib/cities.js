...
module.exports = async function (callbackOrOptions, maybeOptions) {
    ...
    if (typeof callbackOrOptions === 'function') {
        ...
        // Supports calls like (null, options)
        options = maybeOptions;
    }
    ...
    if (!options.googleMapsApiKey) {
        const error = new Error(
            'Please provide a Google Maps API Key and try again.',
        );
    ...
    try {
        ...
        const cities = await extractCitiesFromUrl(
        ...
        for (const lang in config.hfc.website.translatedCitiesJson) {
            ...
            const translatedCities = await extractCitiesFromUrl(
        ...
        for (const lang in config.hfc.website.translatedZonesJson) {
            ...
            const translatedZones = await extractCitiesFromUrl(
        ...
        const segments = await extractCitiesFromSegmentsUrl(
        ...
        for (const lang in config.hfc.website.translatedCitiesJson) {
            ...
            const translatedSegments = await extractCitiesFromSegmentsUrl(
        ...
        for (const cityId in cities) {
            ...
            result.countdown =
                city.countdown != undefined
                    ? city.countdown
                    : await getCountdownForCity(city, options);
...
async function extractCitiesFromSegmentsUrl(url, options) {
    // Execute GET request
    const response = await axios(url, options);
...
async function getCountdownForCity(city, options) {
    ...
    if (typeof result !== 'object') {
        ...
        console.log(
            'Unexpected countdown response for city (defaulting to 0): ' +
                city.name,
        );
        return 0;
    }
    ...
    if (!timeIdentifier) {
        console.log(
            'Unexpected time identifier (defaulting to 0): ' +
                result.time_notes,
        );
        return 0;
    }
